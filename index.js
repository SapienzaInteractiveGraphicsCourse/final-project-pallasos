...
import { FunctionController, GUI } from './jsm/libs/lil-gui.module.min.js';

var start = false; //flag for the animation
var flagL = false; //flag for the light
...
var testaH5, fianchiH5, bustoH5, busto_2H5, busto_3H5, spallaSH5, braccioSH5, abraccioSH5, manoSH5, spallaDH5, gambaSH5, gambaDH5, polSH5, polDH5, piedeDH5; //human5 body
...
var canzone4 = "./js/music/music4.mp3";
var canzone5 = "./js/music/music5.mp3";
...
folder1.add( settings, 'light on' ).onChange( changeL);
...
gui.add( myObject, 'lightintensity1', 0, 1 ).onChange(function(val){
    spotLight.intensity = val
});
gui.add( myObject, 'lightintensity2', 0, 1 ).onChange(function(val){
    spotLight2.intensity = val
});
gui.add( myObject, 'lightintensity3', 0, 1 ).onChange(function(val){
    spotLight3.intensity = val
});
gui.add( myObject, 'lightintensity4', 0, 1 ).onChange(function(val){
...
const colorFormats = {
    //colorspotlight1: spotLight.color.getHex(),
	colorspotlight1: 0xffffff,
	colorspotlight2: 0xffffff,
    colorspotlight3: 0xffffff,
    colorspotlight4: 0x0000ff,
};

gui.addColor( colorFormats, 'colorspotlight1' ).onChange( function ( val ) {

    spotLight.color.setHex( val );

} );

gui.addColor( colorFormats, 'colorspotlight2' ).onChange( function ( val ) {

    spotLight2.color.setHex( val );

} );
gui.addColor( colorFormats, 'colorspotlight3' ).onChange( function ( val ) {

    spotLight3.color.setHex( val );

} );
gui.addColor( colorFormats, 'colorspotlight4' ).onChange( function ( val ) {

    spotLight4.color.setHex( val );
...
cube.userData.grounds = true;
cube.userData.name = "CUBE2"
...
function light(){
    const alight = new THREE.AmbientLight( 0xff00ff,0.2); 
    scene.add( alight );
    ...
    spotLight4 = new THREE.SpotLight( 0x0000ff, 1);
...
//drag ad drop objects
/*
const raycaster = new THREE.Raycaster(); // create once
const clickMouse = new THREE.Vector3();  // create once
const moveMouse = new THREE.Vector3();   // create once
